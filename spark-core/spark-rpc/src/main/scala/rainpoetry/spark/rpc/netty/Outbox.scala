...
import rainpoetry.spark.rpc.client.{RpcResponseCallback, TransportClient}
import rainpoetry.spark.rpc.error.{RpcEnvStoppedException, RpcException}
...
private[netty] class Outbox(nettyEnv: NettyRpcEnv, val address: RpcAddress) {
  ...
  /**
   * Stop [[Outbox]]. The remaining messages in the [[Outbox]] will be notified with
   */
  def stop(): Unit = {
    synchronized {
      if (stopped) {
