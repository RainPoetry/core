...
import protocal.inner.CheckExistence
import rainpoetry.spark.rpc._
import rainpoetry.spark.rpc.client.{RpcResponseCallback, TransportClient, TransportClientBootstrap}
import rainpoetry.spark.rpc.common.{ByteBufferInputStream, ByteBufferOutputStream, ThreadUtils, Utils}
import rainpoetry.spark.rpc.config._
import rainpoetry.spark.rpc.error.{RpcEndpointNotFoundException, RpcEnvStoppedException}
import rainpoetry.spark.rpc.serialize.Serializer
import rainpoetry.spark.rpc.server.{RpcHandler, TransportServer}
...
private[netty] class NettyRpcEnv(
  ...
  private[netty] val transportConf = RpcTransportConfig.fromRpcConf(
    conf,
    "rpc",
   ...
   */
  private val clientFactory = transportContext.createClientFactory(java.util.Collections.emptyList[TransportClientBootstrap])
  ...
  def startServer(bindAddress: String, port: Int): Unit = {
    server = transportContext.createServer(bindAddress, port,  java.util.Collections.emptyList())
  ...
  def asyncSetupEndpointRefByURI(uri: String): Future[RpcEndpointRef] = {
    ...
    verifier.ask[Boolean](CheckExistence(endpointRef.name)).flatMap { find =>
      if (find) {
  ...
  private[netty] def ask[T: ClassTag](message: RequestMessage, timeout: RpcTimeout): Future[T] = {
    ...
    def onFailure(e: Throwable): Unit = {
      if (!promise.tryFailure(e)) {
        e match {
          case e : RpcEnvStoppedException => debug (s"Ignored failure: $e")
  ...
  private[netty] def serialize[T: ClassTag](t: T): ByteBuffer = {
...
private[rpc] class NettyRpcEnvFactory extends RpcEnvFactory with Logging {
  ...
  def create(config: RpcEnvConfig): RpcEnv = {
    val rpcConf = config.config
    // Use JavaSerializerInstance in multiple threads is safe. However, if we plan to support
    // KryoSerializer in future, we have to use ThreadLocal to store SerializerInstance
    val serializer = Utils.classForName(rpcConf.get(SERIALIZE)).getConstructor().newInstance().asInstanceOf[Serializer]
    val nettyEnv =
      new NettyRpcEnv(rpcConf, serializer, config.advertiseAddress, config.cores)
    if (!config.clientMode) {
      ...
      try {
        Utils.startServiceOnPort(config.port, startNettyRpcEnv, rpcConf, config.name)._1
      } catch {
        case NonFatal(e) =>
          nettyEnv.shutdown()
          throw e
...
private[netty] object RequestMessage {
  ...
  def apply(nettyEnv: NettyRpcEnv, client: TransportClient, bytes: ByteBuffer): RequestMessage = {
    ...
    try {
      ...
      ref.client = client
      val newData = bytes.slice();
//      println("remaining: " + newData.position() + " | " + newData.remaining())
      new RequestMessage(
        senderAddress,
        ref,
        // The remaining bytes in `bytes` are the message content.
        nettyEnv.deserialize(client, newData))
    } finally {
      in.close()
    }
  }
...
private[netty] class NettyRpcHandler(
  ...
  override def exceptionCaught(cause: Throwable, client: TransportClient): Unit = {
    ...
    if (addr != null) {
      ...
      error("Exception before connecting to the client", cause)
    }
  }
