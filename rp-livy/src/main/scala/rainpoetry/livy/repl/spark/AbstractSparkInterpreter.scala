...
abstract class AbstractSparkInterpreter extends Interpreter with Logging {
  ...
  protected def postStart(): Unit = {
    bind("spark",
      spark.getClass.getCanonicalName,
      spark,
      List("""@transient"""))
    bind("sc", "org.apache.spark.SparkContext", spark.sparkContext, List("""@transient"""))
  ...
  def config(k: String, v: String): Unit = {
    spark.sessionState.conf.setConfString(k, v)
  }

  def getConf(k: String, default: String): String = {
    spark.conf.get(k, default)
  }
