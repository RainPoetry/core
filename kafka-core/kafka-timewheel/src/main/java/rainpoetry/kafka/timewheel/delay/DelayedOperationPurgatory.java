package rainpoetry.kafka.timewheel.delay;



import rainpoetry.kafka.timewheel.common.utils.Logging;
import rainpoetry.kafka.timewheel.common.utils.Pool;
import rainpoetry.kafka.timewheel.common.utils.ShutdownableThread;
import rainpoetry.kafka.timewheel.common.utils.timer.SystemTimer;
import rainpoetry.kafka.timewheel.common.utils.timer.Timer;
...
import static rainpoetry.kafka.timewheel.common.utils.CoreUtils.*;
...
public class DelayedOperationPurgatory<T extends DelayedOperation> extends Logging {
	...
	public DelayedOperationPurgatory(String purgatoryName) {
		this(purgatoryName, tickMs_default, wheelSize_default);
	}

	public DelayedOperationPurgatory(String purgatoryName, long tickMs, int wheelSize) {
		this(purgatoryName, tickMs, wheelSize, purgeInterval_default, reaperEnable_default, timerEnable_default);
	}

	public DelayedOperationPurgatory(String purgatoryName, long tickMs, int wheelSize,
									 int purgeInterval, boolean reaperEnable,
									 boolean timerEnable) {
		this(purgatoryName, new SystemTimer(purgatoryName, tickMs, wheelSize),
				purgeInterval_default, tickMs * wheelSize, reaperEnable_default, timerEnable_default);
	}

	private DelayedOperationPurgatory(String purgatoryName, Timer timeoutTimer, int purgeInterval,
									  long timeoutMs,
	...
	public boolean tryCompleteElseWatch(T operation, List<Object> watchKeys) {
		if (watchKeys.isEmpty())
			throw new AssertionError("assertion failed: The watch key list can't be empty");
